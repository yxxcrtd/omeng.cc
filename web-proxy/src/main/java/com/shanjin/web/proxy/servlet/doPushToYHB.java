 ...
 */
@WebServlet("/doPushToYHB")
public class doPushToYHB extends HttpServlet {
	private static final long serialVersionUID = 1L;
	static String host = "http://sdk.open.api.igexin.com/apiex.htm";
	private static Logger logger = Logger.getLogger(doPushToYHB.class);
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		System.out.println("");
		System.out.println("=====安卓用户版推送 时间 " + new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date()) + "====");
		System.out.println("YHB AndroidPushInfo:" + AndroidPushInfo);
		System.out.println("YHB transmissionContent:" + transmissionContent);
		System.out.println("YHB pushMap:" + pushMap);
		...
		if (AndroidPushInfo != null && transmissionContent != null && pushMap != null) {
			...
			if (pushMapObject != null) {
				System.out.println("用户版 推送配置 pushMapObject: " + pushMapObject);
				...
				try {
					...
					for (Map<String, Object> pushInfo : AndroidPushInfoObject) {
						System.out.println("用户版 pushInfo:" + pushInfo);
