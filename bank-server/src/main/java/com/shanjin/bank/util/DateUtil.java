...
public class DateUtil {
	...
	public final static String getNowStr() {
		SimpleDateFormat sdf = new SimpleDateFormat();
	 ...
	 * 把日期字符串yyyy-MM-dd HH:mm:ss转换成HH:mm形式
	 */
	public static String strToYMDString(String date) {
		if (date == null || "".equals(date)) {
			return date;
		}
		String temp = "";
		try {
			Date dateStr = DateUtil.parseDate(MONTH_AND_DATE_PATTERN, date);
			temp = DateUtil.formatDate(DATE_YYYYMMDD_PATTERN, dateStr);
		} catch (Exception ex) {
			log.debug(ex.getStackTrace());
		}
		return temp;
