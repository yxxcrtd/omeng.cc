...
import com.shanjin.push.android.PushConfig;
...
//				ms.append(StringUtil.null2Str(msg.get("appType")));
				...
				try{
					...
					System.out.println("异常11："+e.getMessage());
...
//		}
	...
	public  void init (Map<String,Object> configMap,Map<String,Object> msg) {
		...
		if(os.toLowerCase().contains("windows") && Constant.DEVMODE){
			if(pushType==200){//私人助理推送用生产证书
				iphoneCertPath="D:/ios-push-cert/299DisPush.p12";
			}else{
				iphoneCertPath="D:/ios-push-cert/aps_cert.p12";
			}
		}else{
			if(pushType==200){//私人助理推送用生产证书
				iphoneCertPath="/usr/local/299DisPush.p12";
			}
		}
		...
		if(Constant.DEVMODE){
			if(pushType==200){//私人助理推送用生产证书
				iphoneApnsService = APNS.newService().withCert(iphoneCertPath, iphoneCertPassword).withAppleDestination(true).build();	
			}else{
	...
	public  void pushIphoneNotification(String deviceToken,String alert,int badge,boolean soundTip,String message) {
		...
		try {
			...
			System.out.println("异常12："+e.getMessage());
			throw new RuntimeException("推送通知出错");
		}
	}
...
//			}
			...
			System.out.println("异常13："+e.getMessage());
			e.printStackTrace();
