...
Ext.onReady(function() {
	...
	var pieStore = Ext.create("Ext.data.Store",{
        proxy:{  
            type:'ajax',  
            url:'/userTerminal/userChannelPieList',  
            reader:{  
                type:'json',  
                root:'data',  
                idProperty:'ext'  
	...
	var chartStore = Ext.create("Ext.data.Store",{
        proxy:{  
            type:'ajax',  
            url:'/userTerminal/userChannelColList',  
            reader:{  
                type:'json',  
                root:'data',  
                idProperty:'ext'  
    ...
    chartStore.load();  
	 ...
	 var columns = [
	    ...
	    var store = Ext.create("Ext.data.Store",{
	        ...
	        proxy:{  
	            type:'ajax',  
	            url:'/userTerminal/userChannelList',  
	            reader:{  
	                type:'json',  
	                totalProperty:'total',  
	                root:'data',  
	        	...
	        	items:[{xtype : 'datefield',id: 'startTime',name: 'startTime',fieldLabel: '开始日期',format : 'Y-m-d',labelAlign:'left',
	        	       ...
	        	       { xtype: 'button', id:'select',text: '查询',iconCls:'Select',width : 100,listeners: {
	    	    		   click:function(){		  
		           	       ...
		           	       <cms:havePerm url='/userTerminal/exportUserChannelListExcel'>
	 ...
	 // 加载权限
		...
		function exportAll() {
			 var startTime=Ext.getCmp('startTime').getValue();
			 var endTime=Ext.getCmp('endTime').getValue();
	         window.location.href = '/userTerminal/exportUserChannelListExcel?startTime='+startTime+'&endTime='+endTime;	
		};
