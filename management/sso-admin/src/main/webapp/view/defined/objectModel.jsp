...
Ext.onReady(function() {
    var columns = [
                   ...
                   {header:'ID',dataIndex:'id',sortable:true,fixed:false,align:'center',hidden:true},
                   ...
                   {header:'控件区域',dataIndex:'model_type',align:'center',renderer:function(value){  
                       if(value=='1'){  
                           return "<span style='color:red;font-weight:bold';>自定义</span>";  
                       } else if(value=='2'){  
                           return "<span style='color:green;font-weight:bold';>对象</span>";  
                       } else if(value=='3'){  
                           return "<span style='color:blue;font-weight:bold';>固定</span>";  
                       } else if(value=='4'){  
                           return "<span style='color:orange;font-weight:bold';>非必填</span>";  
                       }
           	       }},
                  /** {header:'是否隐藏',dataIndex:'is_hide',align:'center',renderer:function(value){  
           	    ...
           	    {header:'操作',dataIndex:'detail',align:'center',renderer:function(value,v,r){
           	    	...
           	    	if(control_type=='GRID'||control_type=='POP'||control_type=='SWITCH'||control_type=='ARRAY'||control_type=='POPEDIT'){
    ...
    var store = Ext.create("Ext.data.Store",{
        ...
        proxy:{  
            type:'ajax',  
            url:'/defined/getObjectModelList',  
            reader:{  
                type:'json',  
                totalProperty:'total',  
                root:'data',  
 ...
 // 控件类型
  ...
  //点击下一页时传递搜索框值到后台  
    store.on('beforeload', function (store, options) {    
		 ...
		 var new_params = {col_name : col_name,object_id:object_id};    
	...
	items:[
	    	   ...
	    	   <cms:havePerm url='/defined/addObjectModel'>
		       ...
		       <cms:havePerm url='/defined/deleteObjectModel'>
	           { xtype: 'button', id:'del', text: '删除',iconCls:'NewDelete',
			       ...
			       <cms:havePerm url='/defined/editObjectModel'>
      ...
      },
      {
        	...
        	dock:'top',
        	displayInfo: true,
        	items:[{ xtype: 'textfield',id:'object_id',name: 'object_id',fieldLabel: '表Id',value:'${object_id}',labelAlign:'left',labelWidth:60,hidden:true},'-',
        	       { xtype: 'textfield',id:'col_name',name: 'col_name',fieldLabel: '字段名',labelAlign:'left',labelWidth:60,hidden:true},'-',
        	      
        	       <cms:havePerm url='/defined/getObjectModelList'>
        	       { xtype: 'button', id:'select',text: '查询',iconCls:'Select',listeners: {
    	    		   click:function(){
    	    			    var object_id=Ext.getCmp('object_id').getValue();
    	    			   var col_name=Ext.getCmp('col_name').getValue();
    	    			   store.currentPage = 1;
    	    			   store.load({params:{start:0,page:1,limit:20,col_name : col_name,object_id:object_id}}); 
    	    		   }
    	    		   }}
        	        </cms:havePerm>
        	       ]
...
//form表单
            ...
            items:[{ xtype : 'combobox',
				 ...
				 listeners : { // 监听该下拉列表的选择事件
					select : function(combobox,record,index) {
						...
						//add_winForm.form.findField('show_type').setDisabled(true); 	
						//add_winForm.form.findField('show_type').setVisible(false);
					}
				},
				hiddenName:'',
				queryMode : 'local',
                           ...
                           items:[{
                               ...
                               store: Ext.create('Ext.data.Store', {  
                                   fields: [  
                                     ...
                                     data: [  
                                     { 'key':'carBrand','value': '汽车品牌' },  
                                     { 'key':'petType','value': '宠物种类' },
                                     { 'key':'carProvinceId','value': '车牌号' }
                                     ]  
                               }),
                            width:220
               			}]  
	...
	var editform=function(){  
        ...
        edit_winForm.form.findField('link').setValue(rows[0].get('link')+'');
        ...
        edit_winForm.form.findField('unit').setValue(rows[0].get('unit')); 
        edit_winForm.form.findField('model_type').setValue(rows[0].get('model_type')+''); 
...
function showDetail(id,name){
	  ...
	  parent.addTab("object_model_"+id,"字段【"+name+"】详情","Fuwushangyunyingxinxiguanli","/defined/objectModelItemShow?model_id="+id); 	 
}
