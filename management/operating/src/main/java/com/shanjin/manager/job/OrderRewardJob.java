...
public class OrderRewardJob {
	...
	public static void sendReward(Map<String,Object> actMap,String dateStr){
	    ...
	    long s = System.currentTimeMillis();
		try {
			rewardList = Db.find("/*master*/ SELECT t.id,t.order_id,t.merchant_id,t.amount FROM activity_order_reward_detail t WHERE t.is_transfer=0 AND t.activity_id=? AND DATE_FORMAT(t.pay_time,'%Y-%m-%d')=?", actMap.get("activityId"),dateStr);
			...
			if(rewardList!=null&&rewardList.size()>0){
				...
				for(Record r:rewardList){
					...
					prestPayDetail.setLong(3, r.getLong("order_id")); // business_id
					prestPayDetail.setBigDecimal(4, r.getBigDecimal("amount")); // payment_price
					prestPayDetail.setString(5, DateUtil.formatDate(DateUtil.DATE_TIME_PATTERN, new Date()));  // payment_time
	...
	private static void writeMq(List<Record> rewardList) {
		...
		for(Record r:rewardList){
			...
			configMap.put("orderId", r.getLong("order_id"));
			configMap.put("rewardTime", com.shanjin.manager.utils.DateUtil.getDate());
