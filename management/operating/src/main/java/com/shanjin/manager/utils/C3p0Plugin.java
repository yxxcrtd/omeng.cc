...
public class C3p0Plugin implements IPlugin, IDataSourceProvider {
	...
	public C3p0Plugin(String jdbcUrl, String user, String password, String driverClass, Integer maxPoolSize, Integer minPoolSize, Integer initialPoolSize, Integer maxIdleTime, Integer acquireIncrement) {
		initC3p0Properties(jdbcUrl, user, password, driverClass, maxPoolSize, minPoolSize, initialPoolSize, maxIdleTime, acquireIncrement, testConnectionOnCheckin, testConnectionOnCheckout);
	}
	
	private void initC3p0Properties(String jdbcUrl, String user, String password, String driverClass, Integer maxPoolSize, Integer minPoolSize, Integer initialPoolSize, Integer maxIdleTime, Integer acquireIncrement,Boolean testConnectionOnCheckin,Boolean testConnectionOnCheckout) {
		this.jdbcUrl = jdbcUrl;
		this.user = user;
		this.password = password;
		this.driverClass = driverClass != null ? driverClass : this.driverClass;
		...
		this.testConnectionOnCheckin = testConnectionOnCheckin != null ? testConnectionOnCheckin : this.testConnectionOnCheckin;
		this.testConnectionOnCheckout = testConnectionOnCheckout != null ? testConnectionOnCheckout : this.testConnectionOnCheckout;
		System.out.println(this.testConnectionOnCheckin);
	}
	...
	public C3p0Plugin(File propertyfile) {
		...
		try {
			...
			initC3p0Properties(ps.getProperty("jdbcUrl"), ps.getProperty("user"), ps.getProperty("password"), ps.getProperty("driverClass"),
					...
					toBool(ps.getProperty("testConnectionOnCheckin")),toBool(ps.getProperty("testConnectionOnCheckout")));
		} catch (Exception e) {
			e.printStackTrace();
		}
		finally {
