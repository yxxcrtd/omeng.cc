...
public class FensiRewardJob {
	...
	private void handleFensiReward() {
		List<Record> list=Db.find(" /*master*/ select * from activity_fensi_payment_detail where status=0 and merchant_id=147003856059453051 limit 0,1");
		...
		for(Record re:list){
			...
			.set("payment_price", fans_price).set("payment_time", new Date());
			...
			if(compare==1){
				...
				.set("payment_price", rank_price).set("payment_time", new Date());
				Db.save("merchant_payment_details", reDetail);
				
				writeMq(re,1);
			}
			
			re.set("pay_time", new Date()).set("status", 1);
	...
	private void writeMq(Record re,int flag) {
		...
		if (merInfo != null && merInfo.size() > 0) {
			...
			System.out.println(merInfo.get(0).getStr("phone"));
			System.out.println(merInfo.get(0).getStr("name"));
