...
Ext.onReady(function() {
    ...
    var store = Ext.create("Ext.data.Store",{
        ...
        proxy:{  
            type:'ajax',  
            url:'/systemManager/stopKeywordsList',  
            reader:{  
                type:'json',  
                totalProperty:'total',  
                root:'data',  
  ...
  //点击下一页时传递搜索框值到后台  
	...
	displayInfo: true,
	items:[
	    	   <cms:havePerm url='/systemManager/addStopKeywords'>
		       ...
		       <cms:havePerm url='/systemManager/deleteStopKeywords'>
	           { xtype: 'button', id:'del', text: '删除',iconCls:'NewDelete',
			    ...
			    <cms:havePerm url='/systemManager/importStopKeywords'>
		        ...
		        <cms:havePerm url='/systemManager/exportStopKeywords'>
      ...
      },
      {
        	...
        	items:[{ xtype: 'textfield',id:'keyword',name: 'keyword',fieldLabel: '关键词',labelAlign:'left',labelWidth:60},'-',
        	       { xtype: 'numberfield',id:'wordsNum',name: 'wordsNum',fieldLabel: '字数',labelAlign:'left',labelWidth:60},'-',
					'-',
        	       <cms:havePerm url='/systemManager/stopKeywordsList'>
  ...
  //加载数据  
	...
	function exportAll() {
        ...
        window.location.href = '/systemManager/exportStopKeywords?keyword='+keyword+'&wordsNum='+wordsNum;
	};
    ...
    {  
         ...
         Ext.Msg.confirm("提示信息","请确定要执行删除操作吗?",function (btn){  
             ...
             {  
                 Ext.Ajax.request({  
                     url:"/systemManager/deleteStopKeywords",  
                     params:{ids:ids},  
                     method:'post',  
                     success:function(o){   
				...
				handler : function() {
					if (add_winForm.getForm().isValid()) {
						...
						add_winForm.getForm().submit({
							url : '/systemManager/addStopKeywords',
							//等待时显示 等待  
							waitTitle : '请稍等...',
							waitMsg : '正在提交信息...',
						    method : "POST",  
				...
				handler : function() {
					if (import_winForm.getForm().isValid()) {
						import_winForm.getForm().submit({
							url : '/systemManager/importStopKeywords',
							//等待时显示 等待  
							waitTitle : '请稍等...',
							waitMsg : '正在提交信息...',
							success : function(fp, o) {
