...
Ext.onReady(function() {
    var columns = [
                   ...
                   {header:'状态',dataIndex:'applyStatus',sortable:true,width:50,align:'center',fixed:false,renderer:function(value){  
           		 ...
           		 //<cms:havePerm url='/valueAdded/confirmVipMemberStatus'>
                ...
                //	   var str = '';
                	...
                	//   if(status=='0'){
                		   //待确认收款
                	//	   str = '<a href="javascript:void(0);" onclick="confirmApply('+r.data.id+',\''+status+'\')">收款</a>';
                	//   }
                   //	   return str;
                 //  }},
       	         //  </cms:havePerm>
                   <cms:havePerm url='/valueAdded/closeVipMemberStatus'>
                   ...
                   <cms:havePerm url='/valueAdded/openVipMemberStatus'>
                   {id: 'open',  header: '开通功能',  dataIndex: 'open', width:60, sortable:true,align:'center',fixed:false,renderer:function(v,d,r){
                	   var str = '';
                	   var status = r.data.applyStatus;
                       if(status=='0'){
                		   //待开通
        ...
        proxy:{  
            type:'ajax',  
            url:'/valueAdded/vipMemberApplyList',
            reader:{  
                type:'json',  
                totalProperty:'total',  
                root:'data',  
  ...
  //点击下一页时传递搜索框值到后台  
    ...
    var grid = Ext.create("Ext.grid.Panel",{
        ...
        dockedItems: [
            <cms:havePerm url='/valueAdded/deleteVipMemberApply'>
                   {
        	xtype:'toolbar',
        	dock:'top',
        	displayInfo: true,
        	items:[
        	       ...
        	       <cms:havePerm url='/valueAdded/exportVipMemberExcel'>
				...
				labelAlign : 'left'
			     },'-',
			     {
				 ...
				 },'-',
				   {
		            ...
		            <cms:havePerm url='/valueAdded/vipMemberApplyList'>
  ...
  //加载数据  
   ...
   // cityStore.load();
	...
	function exportAll() {
         ...
         window.location.href = '/valueAdded/exportVipMemberExcel?merchantName='+merchantName+'&telephone='+telephone+'&province='+province
   ...
   {  
        ...
        {  
           ...
           Ext.Msg.confirm("提示信息","请确定要执行删除操作吗?",function (btn){  
               ...
               {  
                   Ext.Ajax.request({  
                       url:"/valueAdded/deleteVipMemberApply",  
                       params:{ids:ids},  
                       method:'post',  
                       success:function(o){
...
function confirmApply(id,status){
	var str = "确认已收到商家增值服务费<span style='color:red;font-weight:bold';>5800</span>元?";
    Ext.Msg.confirm("提示信息",str,function (btn){  
        if(btn == 'yes')  
        {  
            Ext.Ajax.request({  
                url:"/valueAdded/confirmVipMemberStatus",  
...
function closeApply(id,status){
    ...
    Ext.Msg.confirm("提示信息",str,function (btn){  
        ...
        {  
            Ext.Ajax.request({  
                url:"/valueAdded/closeVipMemberStatus",  
...
function openApply(id,status){
    ...
    Ext.Msg.confirm("提示信息",str,function (btn){  
        ...
        {  
            Ext.Ajax.request({  
                url:"/valueAdded/openVipMemberStatus",  
