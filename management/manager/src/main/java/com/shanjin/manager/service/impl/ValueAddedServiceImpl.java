...
public class ValueAddedServiceImpl implements IValueAddedService{
	...
	@Override
	public boolean updateOrderPushStatus(String id, int status,float topup_money, Long userId,String userName) {
		return valueAddedDao.updateOrderPushStatus(id, status ,topup_money,userId,userName);
	}
	...
	public List<Pair> getExportVipMemberTitles() {
		...
		titles.add(new Pair("appName", "行业类型"));
		...
		titles.add(new Pair("pay_type", "支付类型"));
		titles.add(new Pair("money", "金额"));
		return titles;
	}
	...
	public Boolean editVipMemberStatus(Map<String, String[]> param) {
		return valueAddedDao.editVipMemberStatus(param);
	}
}
