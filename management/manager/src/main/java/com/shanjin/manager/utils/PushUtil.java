...
public class PushUtil {
	// 本地异常日志记录对象
		...
		public static boolean pushAuditMsg(Long merchantId, String appType, int authStatus) {
			...
			try {
				...
				if(authStatus==1){
					...
					pushType = 15;
					configMap=getConfigurationInfoByKey("pushType-15");
					String data="感谢您参与‘你消费我补贴活动’，10元红包已存入您的钱包余额……";
					paras.put("data", data);
				}
		 ...
		 * 双向补贴消息推送
		 */
		public static boolean pushActivityMsg(Long userId) {
			boolean flag = true;

			try {
				StringBuffer sb = new StringBuffer();
				sb.append(" SELECT up.user_id AS userId,up.client_id AS clientId,up.client_type AS clientType, ");
				sb.append(" up.push_id AS pushId FROM  user_push up where up.user_id= ").append(userId);
				List<Record> merchantClientList = Db.find(sb.toString()); 
				if(merchantClientList==null||merchantClientList.size()<1){
					return flag;
				}
				...
				Map<String, Object> paras = new HashMap<String,Object>();
				Map<String,Object> configMap=null;
				int pushType = 16; // 用户补贴
				
				configMap=getConfigurationInfoByKey("pushType-16");
				String data="感谢您参与‘你消费我补贴活动’，10元红包已存入您的钱包余额……";
				paras.put("pushType", pushType);
				paras.put("data", data);
				PushManager.push(configMap,list, paras,"userId");
