...
public class ActivityDaoImpl implements ActivityDao {
	...
	public List<Record> getOrderRewardAccountList(Map<String, String[]> param) {
        Map<String, String[]> paramMap=Util.getCommonEditParams(param);
		
		if(!(paramMap.get("merchant_name")[0]==null) && !paramMap.get("merchant_name")[0].equals("")){
			paramMap.put("province", null);
			paramMap.put("city", null);
		}
		if(!(paramMap.get("merPhone")[0]==null) && !paramMap.get("merPhone")[0].equals("")){	
			paramMap.put("province", null);
			paramMap.put("city", null);
		}
	...
	private List<OrderRewardAccount> exportOrderRewardAccountbyPhoneExcel(Map<String, String[]> paramMap) {
		...
		sql.append("select mi.name as merchant_name,DATE_FORMAT(aor.pay_time, '%Y-%m-%d %H:%i:%s') as payTime,aor.*,(case aor.is_transfer when 0 then '未到账' when 1 then '已到账' when 2 then '异常' else '未知' end) as isTransfer,(select title from activity_info where id=aor.activity_id) as activity_name,(select service_type_name from service_type where id=aor.service_id) as service_name,(SELECT t.name FROM catalog t WHERE t.alias=mi.app_type and t.level=0) AS app_name"
				...
				+ ",(select (case oi.order_pay_type when 1 then '支付宝支付' when 2 then '微信支付' when 3 then '现金支付' when 4 then '免单' else '' end) as orderPayType from order_info oi where oi.id=aor.order_id) as orderPayType,ui.phone as userPhone ,me.phone as merPhone from activity_order_reward_detail aor inner join merchant_info mi on aor.merchant_id=mi.id "
				+ " join merchant_employees me on me.merchant_id=aor.merchant_id and me.employees_type=1 join user_info ui on ui.id=aor.user_id where ");
		...
		for (int i = 0; i < ids.length; i++) {
			...
			if(re!=null){
				...
				if(is_transfer==1){
					...
					flag = true;
					String activityName=Db.queryStr("select title from activity_info where id=?",re.getLong("activity_id"));
					MessageUtil.createCustomerMessage(null, 1, 1, re.getLong("merchant_id"), activityName, "O盟"+activityName+"奖励您"+re.getBigDecimal("amount")+"元，请查收"); // 生成消息
					final Long merchantId = StringUtil.nullToLong(re.getLong("merchant_id"));
					final String appType = Db.queryStr("select app_type from merchant_info where id=?",re.getLong("merchant_id"));
					new Thread(new Runnable() {			
						@Override
						public void run() {	
							try{
							  PushUtil.pushAuditMsg(merchantId, appType, 2);
							}catch(Exception e){
								e.printStackTrace();
							}
						}
					...
					writeMq(re);
					commonCacheService.deleteObject(CacheConstants.ORDER_REWARD_SURPLUS_MER, DateUtil.formatDate("yyyyMMdd", new Date()),String.valueOf(re.getLong("activity_id")));
				}else if(compare==0){
					re.set("account_time", new Date()).set("is_transfer", 1).set("user_name",name );
					Db.update("activity_order_reward_detail", re);
					flag = true;
	...
	private static void writeMq(Record re) {
		...
		if (merInfo != null && merInfo.size() > 0) {
			...
			configMap.put("orderId", re.getStr("order_no"));
			configMap.put("rewardTime", DateUtil.dateToString(re.getDate("account_time")));
		 ...
		 MqUtil.writeToMQ("orderTemplate", "opay.orderRewardExchange", encryptedText);
			...
			CommonUtil.writeMqFailure(re.getLong("id"), encryptedText,"opay.orderRewardExchange", 2);
			e.printStackTrace();
		} 
	...
	@Override
	public List<Record> getOrderUserRewardAccountList(Map<String, String[]> param) {
		...
		if(!paramMap.get("userPhone")[0].equals("")|| !paramMap.get("merPhone")[0].equals("")){
			return getOrderUserRewardAccountbyPhoneList(paramMap);
		}
		...
		sql.append("select mi.name as merchant_name,aor.*,(select title from activity_info where id=aor.activity_id) as activity_name,(SELECT ROUND((e.attitude_evaluation+e.speed_evaluation+e.quality_evaluation)/3,1) from evaluation e where e.order_id= aor.order_id) as evaluation,(select service_type_name from service_type where id=aor.service_type_id) as service_name,(SELECT t.name FROM catalog t WHERE t.alias=mi.app_type and t.level=0) AS app_name"
				+ ",(select order_pay_type from order_info where id=aor.order_id) as order_pay_type,(select phone from user_info where id=aor.user_id) as userPhone ,(select phone from merchant_employees where merchant_id=aor.merchant_id and employees_type=1 limit 1) as merPhone from activity_user_order_reward aor inner join merchant_info mi on aor.merchant_id=mi.id and mi.is_del=0 where ");
		totalSql.append("select count(1) as total from activity_user_order_reward aor inner join merchant_info mi on aor.merchant_id=mi.id and mi.is_del=0 where ");
		
		strFilter.put("mi.name", paramMap.get("merchant_name"));
		strFilter.put("aor.province", paramMap.get("province"));
		strFilter.put("aor.city", paramMap.get("city"));
		intFilter.put("aor.activity_id", paramMap.get("activity_id"));
		intFilter.put("aor.service_type_id", paramMap.get("service_id"));
		intFilter.put("aor.is_transfer", paramMap.get("is_transfer"));
		strFilter.put("aor.order_no", paramMap.get("order_no"));
		strFilter.put("mi.app_type", paramMap.get("app_type"));
	...
	private List<Record> getOrderUserRewardAccountbyPhoneList(Map<String, String[]> paramMap) {
		...
		sql.append("select mi.name as merchant_name,me.phone as merPhone,ui.phone as userPhone,aor.*,(select title from activity_info where id=aor.activity_id) as activity_name,(SELECT ROUND((e.attitude_evaluation+e.speed_evaluation+e.quality_evaluation)/3,1) from evaluation e where e.order_id= aor.order_id) as evaluation,(select service_type_name from service_type where id=aor.service_type_id) as service_name,(SELECT t.name FROM catalog t WHERE t.alias=mi.app_type and t.level=0) AS app_name"
				+ ",(select order_pay_type from order_info where id=aor.order_id) as order_pay_type from activity_user_order_reward aor inner join merchant_info mi on aor.merchant_id=mi.id and mi.is_del=0 join merchant_employees me on me.merchant_id=aor.merchant_id join user_info ui on ui.id=aor.user_id where ");
		totalSql.append("select count(1) as total from activity_user_order_reward aor inner join merchant_info mi on aor.merchant_id=mi.id and mi.is_del=0 join merchant_employees me on me.merchant_id=aor.merchant_id join user_info ui on ui.id=aor.user_id where ");
	...
	@Override
	public List<OrderUserRewardAccount> exportOrderUserRewardAccountExcel(Map<String, String[]> param) {
		 Map<String, String[]> paramMap=Util.getCommonEditParams(param);
		...
		if(!paramMap.get("userPhone")[0].equals("")|| !paramMap.get("merPhone")[0].equals("")){
			return exportOrderUserRewardAccountbyPhoneExcel(paramMap);
		}
		...
		sql.append("select mi.name as merchant_name,DATE_FORMAT(aor.pay_time, '%Y-%m-%d %H:%i:%s') as payTime,aor.*,(case aor.is_transfer when 0 then '未到账' when 1 then '已到账' when 1 then '已到账' when 2 then '异常' else '未知' end) as isTransfer,(select title from activity_info where id=aor.activity_id) as activity_name,(select service_type_name from service_type where id=aor.service_type_id) as service_name,(SELECT t.name FROM catalog t WHERE t.alias=mi.app_type and t.level=0) AS app_name"
				+ ",(select (case oi.order_pay_type when 1 then '支付宝支付' when 2 then '微信支付' when 3 then '现金支付' when 4 then '免单' else '' end) as orderPayType from order_info oi where oi.id=aor.order_id) as orderPayType,(SELECT ROUND((e.attitude_evaluation+e.speed_evaluation+e.quality_evaluation)/3,1) from evaluation e where e.order_id= aor.order_id) as evaluation "
				+ ",(select phone from user_info where id=aor.user_id) as userPhone ,(select phone from merchant_employees where merchant_id=aor.merchant_id and employees_type=1) as merPhone from activity_user_order_reward aor inner join merchant_info mi on aor.merchant_id=mi.id where ");
		...
		sql.append(",");
		sql.append(pageSize);
		List<OrderUserRewardAccount> orderRewardTotal = OrderUserRewardAccount.dao.find(sql
				.toString());
		
		return orderRewardTotal;
	
	}
	private List<OrderUserRewardAccount> exportOrderUserRewardAccountbyPhoneExcel(Map<String, String[]> paramMap) {
		...
		sql.append("select mi.name as merchant_name,DATE_FORMAT(aor.pay_time, '%Y-%m-%d %H:%i:%s') as payTime,aor.*,(case aor.is_transfer when 0 then '未到账' when 1 then '已到账' when 2 then '异常' else '未知' end) as isTransfer,(select title from activity_info where id=aor.activity_id) as activity_name,(select service_type_name from service_type where id=aor.service_type_id) as service_name,(SELECT t.name FROM catalog t WHERE t.alias=mi.app_type and t.level=0) AS app_name"
				+ ",(SELECT ROUND((e.attitude_evaluation+e.speed_evaluation+e.quality_evaluation)/3,1) from evaluation e where e.order_id= aor.order_id) as evaluation "
				+ ",(select (case oi.order_pay_type when 1 then '支付宝支付' when 2 then '微信支付' when 3 then '现金支付' when 4 then '免单' else '' end) as orderPayType from order_info oi where oi.id=aor.order_id) as orderPayType,ui.phone as userPhone ,me.phone as merPhone from activity_user_order_reward aor inner join merchant_info mi on aor.merchant_id=mi.id "
				+ " join merchant_employees me on me.merchant_id=aor.merchant_id and me.employees_type=1 join user_info ui on ui.id=aor.user_id where ");
	...
	public boolean delOrderUserReAccount(Map<String, String[]> param, String operUserName) {
		...
		String[] ids = param.get("id")[0].split(",");
		for (int i = 0; i < ids.length; i++) {
			re=Db.findById("activity_user_order_reward", ids[i]);
			if(re!=null){
				int is_transfer=re.getInt("is_transfer");
				BigDecimal amount=re.getBigDecimal("amount");
				...
				if(is_transfer==1){
					...
					re.set("account_time", new Date()).set("is_transfer", 1).set("user_name",operUserName );
					Db.update("activity_user_order_reward", re);
					flag = true;
					String activityName=Db.queryStr("select title from activity_info where id=?",re.getLong("activity_id"));
					
					MessageUtil.createCustomerMessage(null, 1, 2, re.getLong("user_id"), activityName, "O盟"+activityName+"奖励您"+re.getBigDecimal("amount")+"元，请查收"); // 生成消息
					final Long userId = StringUtil.nullToLong(re.getLong("user_id"));
					new Thread(new Runnable() {			
						@Override
						public void run() {	
							try{
							  PushUtil.pushActivityMsg(userId);
							}catch(Exception e){
								e.printStackTrace();
							}
						}
					...
					commonCacheService.deleteObject(CacheConstants.ORDER_REWARD_SURPLUS, DateUtil.formatDate("yyyyMMdd", new Date()),String.valueOf(re.getLong("activity_id")));
				}else if(compare==0){
					re.set("account_time", new Date()).set("is_transfer", 1).set("user_name",operUserName );
					Db.update("activity_user_order_reward", re);
					flag = true;
	...
	private void writeBUtieMq(Record re) {
		String msg="";
		String encryptedText="";
		try {
		String phone=Db.queryStr("select phone from user_info where id=?",re.getLong("user_id"));
		Map<String, Object> configMap = new HashMap<String, Object>();
		configMap.put("merchantUserId", re.getLong("user_id"));
		configMap.put("merchantUserPhone", phone);
		configMap.put("userName", "");
		configMap.put("activityName", "双向补贴");
		configMap.put("activityId", re.getLong("activity_id"));
		configMap.put("transSeq", re.getLong("id"));
		configMap.put("rewardTime",DateUtil.dateToString(re.getDate("account_time")));
		configMap.put("transAmount", re.getBigDecimal("amount"));
		configMap.put("remark", "双向补贴奖");
	...
	@Override
	public boolean editOrderUserRewardAccount(Map<String, String[]> param, String operUserName) {

		boolean flag=false;
		String id=param.get("id")[0];
		String is_transfer=param.get("is_transfer")[0];
		String remark=param.get("remark")[0];
		Record re=Db.findById("activity_user_order_reward", id);
		if(re!=null){
			if(re.getInt("is_transfer")==1){
				return flag;
			}
