            ...
            items:[{  
                   items:[{xtype: 'button', id:'select1',text: 'appType',width : 70,listeners: {
          		   ...
          		   {xtype: 'button', id:'select3',text: '城市',width : 60,listeners: {
          			...
          			{xtype: 'button', id:'select4',text: '订单状态',width : 70,listeners: {
                   		   click:function(){	
                 			     var dimension='order_status';
                 			    store1.load({params:{dimension:dimension}}); 
...
//列表详情
    ...
    var columns = [
                   ...
                   {header:'ID',dataIndex:'id',align:'center',sortable:true,fixed:false,hidden:true,width : 300},
                   {header:'订单数量',itemId: 'terminalNum',align:'center',dataIndex:'terminalNum',sortable:true,fixed:false,width : 300},
                   {header:'订单状态',itemId: 'order_status',align:'center',dataIndex:'order_status',sortable:true,fixed:false,hidden:true,width :300,renderer:function(value){  
                       if(value=='0'){  
                           return "<span style='font-weight:bold';>已取消订单</span>";  
                       } else if(value=='1'){  
                           return "<span style='font-weight:bold';>新预约</span>";  
                       }else if(value=='2'){  
                           return "<span style='font-weight:bold';>待选择</span>";  
                       }else if(value=='3'){  
                           return "<span style='font-weight:bold';>服务中</span>";  
                       }else if(value=='4'){  
                           return "<span style='font-weight:bold';>待支付</span>";  
                       }else if(value=='5'){  
                           return "<span style='font-weight:bold';>服务完成</span>";  
                       }else if(value=='6'){  
                           return "<span style='font-weight:bold';>订单已过期</span>";  
                       }
           		   }},
                   {header:'app类型',itemId: 'app_type',align:'center',dataIndex:'app_type',sortable:true,fixed:false,hidden:true,width : 300},
                   {header:'省份',itemId: 'province',align:'center',dataIndex:'province',sortable:true,fixed:false,hidden:true,width : 300},
                   {header:'城市',itemId: 'city',align:'center',dataIndex:'city',sortable:true,fixed:false,hidden:true,width : 300}
                   ];
    ...
    var store = Ext.create("Ext.data.Store",{
        ...
        proxy:{  
            type:'ajax',  
            url:'/statistic/getUserOrderList',  
            reader:{  
                type:'json',  
                totalProperty:'total',  
                root:'data',  
 ...
 // app类型
	...
	var storeOrderStatus = Ext.create("Ext.data.Store", {
		pageSize : 20, // 每页显示几条数据
		proxy : {
			type : 'ajax',
    ...
    store.on('beforeload', function (store, options) {    
    	...
    	var order_status=Ext.getCmp('order_status_Filter').getValue();
		...
		var new_params = { dimension:dimension,app_type : app_type,province:province,city:city,order_status:order_status,start_time:start_time,off_time:off_time};    
    ...
    var grid = Ext.create("Ext.grid.Panel",{
        ...
        dockedItems: [{
        	...
        	displayInfo: true,
        	items:['-',
        	       <cms:havePerm url='/statistic/exportUserOrderListExcel'>
   ...
   
      {
        	...
        	items:[{xtype : 'combobox',id : 'app_type_Filter',name : 'appType',fieldLabel : 'app类型',value:'',valueField : 'app_type',displayField : 'app_name',width : 250,
				     ...
				     {xtype : 'combobox',id : 'order_status_Filter',name : 'order_status',fieldLabel : '订单状态',valueField : 'dict_key',displayField : 'dict_value',editable:false,
						store : storeOrderStatus,queryMode : 'local',labelAlign : 'left',labelWidth : 60
						},'-',
        	...
        	items:[
				   ...
				   <cms:havePerm url='/statistic/getUserOrderList'>
        	       { xtype: 'button', id:'select',text: '查询',iconCls:'Select',width : 100,listeners: {
    	    		   click:function(){		  
    	    				...
    	    				var off_time=Ext.getCmp('off_time').getValue();
    	    				store.currentPage = 1;
     	    			    store.load({params:{start:0,page:1,limit:20,dimension:dimension,app_type : app_type,order_status:order_status,province:province,city:city,start_time:start_time,off_time:off_time}}); 
    	    		   }
    	    		   }}
				   </cms:havePerm>
				   ]
 ...
 // 加载权限
  ...
  //导出
	function exportAll() {
		...
		var start_time=Ext.getCmp('start_time').getValue();
		var off_time=Ext.getCmp('off_time').getValue();
           window.location.href = '/statistic/exportUserOrderListExcel?dimension='+dimension+'&start_time='+start_time+'&off_time='+off_time
           +'&app_type='+app_type+'&province='+province+'&city='+city+'&order_status='+order_status;	
	};
