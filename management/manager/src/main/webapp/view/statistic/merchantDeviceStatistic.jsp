...
Ext.onReady(function() {
	...
	var pieStore = Ext.create("Ext.data.Store",{
        proxy:{  
            type:'ajax',  
            url:'/merchantTerminal/merchantDevicePieList',  
            reader:{  
                type:'json',  
                root:'data',  
                idProperty:'ext'  
	...
	var chartStore = Ext.create("Ext.data.Store",{
        proxy:{  
            type:'ajax',  
            url:'/merchantTerminal/merchantDeviceColList',  
            reader:{  
                type:'json',  
                root:'data',  
                idProperty:'ext'  
    ...
    chartStore.load();  
	 ...
	 var columns = [
	                   ...
	                   {header:'设备名称',itemId: 'model',align:'center',dataIndex:'model',sortable:true,fixed:false},
	    ...
	    var store = Ext.create("Ext.data.Store",{
	        ...
	        proxy:{  
	            type:'ajax',  
	            url:'/merchantTerminal/merchantDeviceList',  
	            reader:{  
	                type:'json',  
	                totalProperty:'total',  
	                root:'data',  
	    ...
	    var grid = Ext.create("Ext.grid.Panel",{
	   ...
	   
	      {
	        	...
	        	dock:'top',
	        	displayInfo: true,
	        	items:[{ xtype: 'textfield',id: 'model',name: 'model',fieldLabel: '设备名称',labelAlign:'left',labelWidth:60},'-',
	        	       { xtype: 'button', id:'select',text: '查询',iconCls:'Select',width : 100,listeners: {
	    	    		   click:function(){		  
	    	    				var model=Ext.getCmp('model').getValue();
	    	    				store.currentPage = 1;
	     	    			    store.load({params:{start:0,page:1,limit:20,model:model}}); 
	    	    		   }
	    	    		   }}
					   ]}],
