...
Ext.onReady(function() {
	...
	var modelMenu = Ext.define("TreeModel", { // 定义树节点数据模型
			...
			fields :[{name : "id",type : "int"},
					...
					{name : "big_icon_path",type : "string"},
       ...
       msg='撤回';
         ...
         {  
            ...
            Ext.Msg.confirm("提示信息","确定要"+msg+"分类吗?",function (btn){  
                ...
                {  
                    Ext.Ajax.request({  
                        url:"/slider/releaseOrrecallCatalog",  
  ...
  //form表单
    function showNodeForm(record,flag){ 
    	...
    	var bigImg = '';
 		if(flag==1){
 			smallImg = record.data.icon_path;
 			bigImg = record.data.big_icon_path;
 		}
    	    ...
    	    //================判断是编辑还是新增===============
    	    	if(flag==1){
    	    	    ...
    	    	    nodeForm.form.findField('alias').setValue(record.data.alias);
    	    	    ...
    	    	    nodeForm.form.findField('icon_path').setValue(record.data.icon_path);
    	    	    nodeForm.form.findField('big_icon_path').setValue(record.data.big_icon_path);
    	        	title = '编辑节点';
    	        	reqName = 'editCatalog';
    	    	}else if(flag==2){
...
var addMenuTab=function(record) {
	...
	var leafPannel = Ext.getCmp("leafNode"+id);
		 ...
		 if(leafPannel==undefined){
			 ...
			 closable : true,
			 iconCls : icon,
			 html : '<iframe width="100%" height="100%" frameborder="0" src="/slider/catalogServiceIndex?catalogid='+id+'"></iframe>'
			 });
