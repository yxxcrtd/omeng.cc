...
<form id="exportForm" action="/report/htmlToExcel" method="post" style="display: inline;">
    <input type="hidden" name="filename" value="WeixinBill">
...
<div id="divReport">
    <table class="bordered">
        ...
        <c:forEach var="obj" items="${list}" varStatus="s">
            <tr>
                <td>${s.index+1}</td>
                <td class="x-num">${obj.get("wx_no")}</td>
                <td class="x-money">${obj.get("wx_amount")}</td>
                <td class="x-date"><fmt:formatDate value="${obj.get(\"wx_date\")}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
