...
<form id="exportForm" action="/report/htmlToExcel" method="post" style="display: inline;">
    <input type="hidden" name="filename" value="AlipayBill">
    <textarea name="html" style="display: none"></textarea>
</form>
<div id="toolbar"></div>
<div class="tooltip" style="color: red;display:none;">提示：账单明细中存在不平衡的数据，请及时检查原因，并调整正确。</div>
<div id="divReport">
    <table class="bordered">
        <thead>
        <tr>
            <th></th>
            <th colspan="3">支付宝平台数据</th>
            <th colspan="4">O盟平台数据</th>
            <th></th>
        </tr>
        <tr>
            <th>序号</th>
        ...
        <c:forEach var="obj" items="${list}" varStatus="s">
            <tr>
                <td>${s.index+1}</td>
                <td class="x-num">${obj.get("ali_no")}</td>
                <td class="x-money">${obj.get("ali_amount")}</td>
                <td class="x-date"><fmt:formatDate value="${obj.get(\"ali_date\")}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
                <td class="x-num">${obj.get("omeng_no")}</td>
                <td>${obj.get("omeng_type")}</td>
                <td  class="x-money">${obj.get("omeng_amount")}</td>
                <td  class="x-date"><fmt:formatDate value="${obj.get(\"omeng_date\")}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
                <td>
                    <c:if test="${obj.get('result') == 0}">
                        <span style="color: red;">不平衡</span>
                        <script>$("#tooltip").show()</script>
                    </c:if>
