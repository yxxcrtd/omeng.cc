...
Ext.onReady(function() {
    var columns = [
                   ...
                   {header:'审核状态',dataIndex:'is_audit',renderer:function(value){  
                       if(value=='0'){  
                           return "<span style='color:red;font-weight:bold';>未审核</span>";  
                       } else if(value=='1'){  
                           return "<span style='color:green;font-weight:bold';>已通过</span>";  
                       }else if(value=='2'){
                    	   return "<span style='color:blue;font-weight:bold';>未通过</span>";  
                       }
           	       }},
           	       { id: 'certificate',  header: '审核',  dataIndex: 'is_audit',  sortable:true,fixed:false,renderer:function(value,v,r){
                	  var app_key_word = r.data.app_key_word;
      	              var remark = r.raw.remark;
      	              var join_time=r.data.join_time;
    	              var is_audit= r.data.is_audit;
    	              if(value=='0'){
              	        return '<a href="javascript:audit(\''+app_key_word+'\',\''+ join_time+'\',\''+ remark+'\',\''+is_audit +'\')">审核</a>';
              	      }else if(value=='2'){
              		    return '<a href="javascript:audit(\''+app_key_word+'\',\''+ join_time+'\',\''+ remark+'\',\''+is_audit +'\')"><span style="color:red;font-weight:bold";>拒绝原因</span></a>'; 
              	      }else{
              		    return '';  
              	      }
                   }}
    ...
    store = Ext.create("Ext.data.Store",{
        ...
        proxy:{  
            type:'ajax',  
            url:'/systemManager/serviceWordsList',
            reader:{  
                type:'json',  
                totalProperty:'total',  
                root:'data',  
  ...
  //点击下一页时传递搜索框值到后台  
    ...
    var grid = Ext.create("Ext.grid.Panel",{
        ...
        dockedItems: [{
        	...
        	items:[
        	       '-',
        	       <cms:havePerm url='/systemManager/deletetServiceWord'>
                   { xtype: 'button', id:'del', text: '删除',iconCls:'NewDelete',
        	    	   ...
        	    	   <cms:havePerm url='/systemManager/exportServiceWord'>
 	       ...
 	       { xtype: 'numberfield',id:'wordsNum',name: 'wordsNum',fieldLabel: '字数',labelAlign:'left',labelWidth:60},'-',
        	       ...
        	       <cms:havePerm url='/systemManager/serviceWordsList'>
  ...
  //加载数据  
	...
	function exportAll() {
    	 ...
    	 var start_time=Ext.getCmp('start_time').getValue();
		 var off_time=Ext.getCmp('off_time').getValue();
         window.location.href = '/systemManager/exportServiceWord?app_key_word='+app_key_word+'&wordsNum='+wordsNum+'&start_time='+start_time+'&off_time='+off_time;
	};
	//订单删除
	 function delUserAll()  
    {  
         ...
         {  
            ...
            Ext.Msg.confirm("提示信息","请确定要执行删除操作吗?",function (btn){  
                ...
                {  
                    Ext.Ajax.request({  
                        url:"/systemManager/deletetServiceWord",  
	                             ...
	                             Ext.Msg.confirm("提示信息","确定审核通过吗?",function (btn){  
	                              ...
	                              {  
	  	                            	cert_winForm.getForm().submit({  
	  	                                          url :'/systemManager/AuditServiceWord',  
	  	                                           //等待时显示 等待  
	  	                                          waitTitle: '请稍等...',  
	  	                                          waitMsg: '正在提交信息...',  
	 		    ...
	 		    handler : function(){
                    ...
                    var txt=formPanel.form.findField('txt').getRawValue();
	              	cert_winForm.form.findField('remark').setValue(txt);
	              	  cert_winForm.getForm().submit({  
	                      url :'/systemManager/RefuseServiceWord',  
	                       //等待时显示 等待  
	                      waitTitle: '请稍等...',  
	                      waitMsg: '正在提交信息...',  
