...
//        });
        var store = Ext.create("Ext.data.Store", {
            ...
            proxy: {
                ...
                {name: 'fans_count', type: 'string'}, //mapping:0 这样的可以指定列显示的位置，0代表第1列，可以随意设置列显示的位置
                {name: 'fans_price'},
                {name: 'rank'},
                {name: 'rank_price'},
                {name: 'pay_time'},
        ...
        store.on('beforeload', function (store, options) {
            ...
            if(txtBeginDate.getValue())
                params.beginDate = Ext.Date.format(new Date(txtBeginDate.getValue()),'Y-m-d');
            if(txtEndDate.getValue())
                params.endDate = Ext.Date.format(new Date(txtEndDate.getValue()),'Y-m-d');
        ...
        Ext.create('Ext.grid.Panel', {
            ...
            columns: [
                ...
                {header: '服务商', dataIndex: 'name'},
                {header: '手机号', dataIndex: 'telephone'},
                {header: '省份', dataIndex: 'province'},
                {header: '城市', dataIndex: 'city'},
                {header: '行业类型', dataIndex: 'app_name'},
                {header: '粉丝数', dataIndex: 'fans_count'},
                {header: '粉丝数排名', dataIndex: 'rank'},
                {header: '粉丝数奖励', dataIndex: 'fans_price'},
                {header: '排名奖励', dataIndex: 'rank_price'},
//                {header: '支付状态', dataIndex: 'status', renderer: function(val){
//                    if(val == 0) return "未支付";
//                    else if(val == 1) return "已支付";
//                }},
                {header: '奖励发放时间', dataIndex: 'pay_time'}
            ],
            width: '100%',
