...
import static com.shanjin.common.util.DateUtil.DATE_YYYYMMDDHHMMSSSSS_PATTER;
...
public class DateUtil {
	...
	public static boolean checkOrderTime(Map<String, Object> params) {
		if(!params.containsKey("isImmediate") || "0".equals(params.get("isImmediate").toString())){//立即服务，不需要验证时间
			String date = "";
			Date nDate = new Date();
			Date cDate = new Date();
	...
	/**
	 * 获取当前时间前或者后N天日期，时间格式自定
	 * @return
	 */
	public static String getAfterDateN(String currentTime,int days,String format) {
	 ...
	 * @return
	 */
	public static String getDateTimeByMinuts(String time,int minutes) {
	...
	public static boolean lessThanTime(Date date) {
        Date currTime=new Date();
        long diff =currTime.getTime()-date.getTime();
	...
	public static String getYesterdayChina() {
        Date date=new Date();//取时间
        Calendar calendar = new GregorianCalendar();
        calendar.setTime(date);
        calendar.add(calendar.DATE,-1);//把日期往后增加一天.整数往后推,负数往前移动
	 ...
	 * @return
	 */
	public static Long getMicorSeconds(String aMask, String strDate){
		 Date DateServiceTime=null;
			try {
				DateServiceTime = DateUtil.convertStringToDate(DateUtil.DATE_TIME_YYYY_MM_DD_HH_MM_PATTERN,strDate);
			} catch (ParseException e) {
				e.printStackTrace();
			}
			if (DateServiceTime==null){
				 return -1L;
			}else{
				 return DateServiceTime.getTime();
			}
		
	}
	
	public static String getTimestampToString(Timestamp time,String format){
	...
	public static long getMillisByDate(String date,String format){
	  SimpleDateFormat sdf = new SimpleDateFormat(format);
	//毫秒
	...
	*/
	public static String showTime(Date ctime, String format) {
		...
		long nowtimelong = System.currentTimeMillis() / (long) 1000;
		long ctimelong = ctime.getTime() / (long) 1000;
...
//		
//		System.out.println(getDaysBetweenDAY1andDAY2("2016-10-20", "2016-10-22"));
//		
//		System.out.println(DateUtil.getTimeFromBetweenTimes(new Date().getTime(),DateUtil.parseDate(DATE_TIME_PATTERN, "2015-10-27 10:41:00").getTime()));
//		
//		System.out.println(formatLongToTxt(new Date().getTime()-DateUtil.parseDate(DATE_TIME_PATTERN, "2016-10-27 10:41:00").getTime()));

		System.out.println(DateUtil.showTime(DateUtil.parseDate(DATE_TIME_PATTERN, "2016-9-28 13:41:00")));
	}
}
